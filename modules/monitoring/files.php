...
function monitoring_file_categories(): array
{
    return [
...
function monitoring_sanitize_category(string $category): string
{
    $allowed = monitoring_file_categories();
    $key = strtolower(trim($category));
    return array_key_exists($key, $allowed) ? $key : '';
}

function monitoring_sanitize_year($year, int $minYear, int $maxYear): ?int
{
...
function monitoring_redirect(string $category, int $year, string $status = ''): void
{
    $params = [
        'category' => $category,
        'year' => (string)$year,
    ...
    header('Location: ' . url_with_base('modules/monitoring/files.php?' . http_build_query($params)));
    exit;
}

function monitoring_bytes_label(int $bytes): string
{
    if ($bytes < 1024) {
        return $bytes . ' B';
    }
...
$categories = monitoring_file_categories();
$categoryKeys = array_keys($categories);
$defaultCategory = $categoryKeys[0] ?? 'inspection';
$canManage = can('monitoring.manage');
$statusCode = '';

$selectedCategory = monitoring_sanitize_category((string)($_GET['category'] ?? ''));
if ($selectedCategory === '') {
    $selectedCategory = $defaultCategory;
}

$categoryCounts = array_fill_keys(array_keys($categories), 0);
$catRes = $conn->query("SELECT category_key, COUNT(*) AS total FROM monitoring_files WHERE is_deleted = 0 GROUP BY category_key");
if ($catRes instanceof mysqli_result) {
    while ($row = $catRes->fetch_assoc()) {
        $key = monitoring_sanitize_category((string)($row['category_key'] ?? ''));
        if ($key !== '') {
...
$selectedYear = monitoring_sanitize_year($_GET['year'] ?? null, $minYear, $maxYear);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_csrf();
    require_rate_limit('monitoring:post:' . (int)($_SESSION['user_id'] ?? 0), 10, 600);
    ...
    if ($action === 'upload') {
        $postCategory = monitoring_sanitize_category((string)($_POST['category'] ?? ''));
        $postYear = monitoring_sanitize_year($_POST['file_year'] ?? null, $minYear, $maxYear);

        if ($postCategory === '' || $postYear === null) {
            monitoring_redirect($selectedCategory, $selectedYear, 'invalid_target');
        }

        if (!isset($_FILES['monitoring_file']) || !is_array($_FILES['monitoring_file'])) {
            monitoring_redirect($postCategory, $postYear, 'upload_missing');
        }
        ...
        $check = validate_upload(
            $_FILES['monitoring_file'],
            $allowedExt,
            $allowedMime,
            20 * 1024 * 1024
        );
        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $mime = $finfo->file((string)$_FILES['monitoring_file']['tmp_name']) ?: '';
        if (empty($check['ok']) || $check['size'] > 20 * 1024 * 1024 || !in_array($mime, $allowedMime, true)) {
            monitoring_redirect($postCategory, $postYear, 'upload_invalid');
        }

        $uploadAbsDir = $_SERVER['DOCUMENT_ROOT'] . url_with_base('uploads/monitoring/' . $postCategory . '/' . $postYear . '/');
        $uploadRelDir = url_with_base('uploads/monitoring/' . $postCategory . '/' . $postYear . '/');
        if (!is_dir($uploadAbsDir) && !mkdir($uploadAbsDir, 0755, true) && !is_dir($uploadAbsDir)) {
            monitoring_redirect($postCategory, $postYear, 'upload_dir_error');
        }

        $rawOriginal = basename((string)($_FILES['monitoring_file']['name'] ?? 'document'));
        ...
        if (!move_uploaded_file((string)$_FILES['monitoring_file']['tmp_name'], $destAbs)) {
            monitoring_redirect($postCategory, $postYear, 'upload_failed');
        }
        ...
        if (!$stmt) {
            @unlink($destAbs);
            monitoring_redirect($postCategory, $postYear, 'db_error');
        }
        ...
        monitoring_redirect($postCategory, $postYear, 'upload_ok');
    }
    ...
    if ($action === 'delete') {
        ...
        if ($postCategory === '' || $postYear === null || $rowId <= 0) {
            monitoring_redirect($selectedCategory, $selectedYear, 'delete_invalid');
        }

        $stmt = $conn->prepare(
            "UPDATE monitoring_files
        ...
        monitoring_redirect($postCategory, $postYear, $changed > 0 ? 'delete_ok' : 'delete_missing');
    }
}
...
$flashMap = [
    ...
    'table_error' => ['type' => 'danger', 'text' => 'Monitoring files table is not ready. Please check database connection.'],
];
...
<div class="page-wrapper monitoring-files-page">
  <div class="page-breadcrumb">
    <div class="row">
      <div class="col-6 align-self-center">
        <h4 class="page-title fw-semibold text-dark mb-0">Monitoring Folder</h4>
      </div>
      <div class="col-6 align-self-center">
        <div class="d-flex align-items-center justify-content-end gap-2">
          <a href="<?= url_with_base('dashboard.php') ?>" class="btn monitoring-back-btn">Back</a>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent mb-0 p-0">
              <li class="breadcrumb-item"><a href="<?= url_with_base('dashboard.php') ?>" class="text-primary">Home</a></li>
              <li class="breadcrumb-item active text-muted" aria-current="page">Monitoring Folder</li>
            </ol>
          </nav>
        </div>
      </div>
  ...
  <div class="container-fluid">
    <?php if ($flash): ?>
      <div class="alert alert-<?= htmlspecialchars($flash['type'], ENT_QUOTES, 'UTF-8') ?> mb-3 monitoring-flash-alert" role="alert">
        <?= htmlspecialchars($flash['text'], ENT_QUOTES, 'UTF-8') ?>
      </div>
    <?php endif; ?>

    <div class="card monitoring-files-card mb-3">
      <div class="card-body">
        <div class="monitoring-toolbar mb-3">
          <ul class="nav nav-pills monitoring-category-tabs">
            <?php foreach ($categories as $key => $label): ?>
              <li class="nav-item">
                <a class="nav-link <?= $selectedCategory === $key ? 'active' : '' ?>"
                   href="<?= url_with_base('modules/monitoring/files.php?category=' . urlencode($key) . '&year=' . (int)$selectedYear) ?>">
                  <?= htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?>
                  <span class="monitoring-badge"><?= (int)($categoryCounts[$key] ?? 0) ?></span>
                </a>
              </li>
            <?php endforeach; ?>
          </ul>
          ...
          <?php if ($canManage): ?>
            <form method="POST" enctype="multipart/form-data" class="monitoring-upload-inline">
              <?= csrf_input(); ?>
              ...
              <input type="file" name="monitoring_file" id="monitoringFileInput" class="monitoring-file-input-hidden" required>
              <button type="button" class="btn monitoring-pick-btn" id="monitoringPickBtn">Select File</button>
              <span class="monitoring-selected-file" id="monitoringSelectedFile" aria-live="polite">No file selected</span>
              <button type="submit" class="btn monitoring-upload-btn" id="monitoringUploadBtn" disabled>Upload</button>
            </form>
          <?php endif; ?>
        </div>

        <div class="monitoring-year-row">
          <ul class="nav nav-pills monitoring-year-tabs">
            <?php foreach ($years as $year): ?>
              <li class="nav-item">
                <a class="nav-link <?= (int)$selectedYear === (int)$year ? 'active' : '' ?>"
                   href="<?= url_with_base('modules/monitoring/files.php?category=' . urlencode($selectedCategory) . '&year=' . (int)$year) ?>">
                  <?= (int)$year ?>
                  <span class="monitoring-year-pill-count"><?= (int)($yearCounts[$year] ?? 0) ?></span>
                </a>
              </li>
            <?php endforeach; ?>
          </ul>

          <div class="monitoring-year-select-wrap">
            <label for="monitoringYearSelect" class="visually-hidden">Select year</label>
            <select id="monitoringYearSelect" class="form-select form-select-sm" data-category="<?= htmlspecialchars($selectedCategory, ENT_QUOTES, 'UTF-8') ?>">
              <?php foreach ($years as $year): ?>
    ...
    <div class="card monitoring-files-card">
      <div class="card-body">
        <h5 class="mb-3">
        ...
        <?php else: ?>
          <div class="table-responsive">
            <table id="monitoringFilesTable" class="table table-striped table-hover align-middle mb-0">
              <thead>
                <tr>
              ...
              <tbody>
                <?php foreach ($files as $row): ?>
                  <tr>
                    <td><?= (int)$row['id'] ?></td>
                    <td>
                      <a href="<?= htmlspecialchars((string)$row['file_path'], ENT_QUOTES, 'UTF-8') ?>" target="_blank" rel="noopener noreferrer" class="monitoring-file-link">
                      ...
                      <div class="small text-muted">
                        ...
                        <?= htmlspecialchars(monitoring_bytes_label((int)($row['file_size'] ?? 0)), ENT_QUOTES, 'UTF-8') ?>
                      </div>
                    </td>
...
<script>
  $(function () {
    ...
    const $flash = $('.monitoring-flash-alert');
    if ($flash.length) {
      setTimeout(function () {
    ...
    const fileInput = document.getElementById('monitoringFileInput');
    const pickBtn = document.getElementById('monitoringPickBtn');
    const selectedFile = document.getElementById('monitoringSelectedFile');
    const uploadBtn = document.getElementById('monitoringUploadBtn');
    ...
    const yearSelect = document.getElementById('monitoringYearSelect');
    if (yearSelect) {
      yearSelect.addEventListener('change', function () {
        const category = this.getAttribute('data-category') || '';
        const year = this.value || '';
        if (!category || !year) return;
        window.location.href = (window.BASE_URL || '') + '/modules/monitoring/files.php?category=' + encodeURIComponent(category) + '&year=' + encodeURIComponent(year);
      });
    }

    const $table = $('#monitoringFilesTable');
